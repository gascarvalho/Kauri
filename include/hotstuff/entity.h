...
namespace hotstuff
{
    ...
    struct ReplicaInfo
    {
        ...
        ReplicaInfo(ReplicaID id,
                    ...
                    pubkey_bt &&pubkey) : id(id), peer_id(peer_id), pubkey(std::move(pubkey)) {}

        ReplicaInfo(const ReplicaInfo &other) : id(other.id), peer_id(other.peer_id),
                                                pubkey(other.pubkey->clone()) {}

        ReplicaInfo(ReplicaInfo &&other) : id(other.id), peer_id(other.peer_id),
    ...
    public:
        ...
        Block(bool delivered, int8_t decision) : qc(new QuorumCertDummy()),
        ...
        Block(const std::vector<block_t> &parents,
              ...
              quorum_cert_bt &&self_qc,
              int8_t decision = 0) : parent_hashes(get_hashes(parents)),
                                     cmds(cmds),
                                     qc(std::move(qc)),
