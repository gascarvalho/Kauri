"""Safe CLI-driver tests for the prospective one-shot SHAPE32 campaign."""
...
MANIFEST = REPOSITORY / "experiments/adaptive/profiles/shape-placement-factorial-v32.json"
V31_MANIFEST = (
...
def candidate_v32_identities():
    """Independently recompute prospective v32 identities without freezing them."""
...
def frozen_contract(candidate_v32_identities):
...
def test_smoke_preflight_rejects_runtime_identity_drift(
    ...
    refusal = json.loads(capsys.readouterr().err)
    assert refusal == {
        "reason": "smoke runtime bytes differ from the exact frozen v32 identity",
        "status": "REJECT",
    }
...
def test_coverage_smoke_preflight_validates_exact_n31_runtime(
    ...
    assert result["runtime_id"] == (
        "shape-placement-factorial-v32-excluded-n31-coverage-smoke-v1"
    )
...
def test_run_requires_explicit_authorization_before_any_launch(
    ...
    assert not (repository / "results/shape-placement-factorial-v32").exists()
...
def test_smoke_runtime_identity_drift_rejects_before_result_claim(
    ...
    assert not (
        repository / "results/shape-placement-factorial-v32-smoke"
    ).exists()
...
def test_coverage_smoke_runtime_identity_drift_rejects_before_result_claim(
    ...
    assert not (
        repository / "results/shape-placement-factorial-v32-coverage-smoke"
    ).exists()
...
def test_smoke_generates_exact_excluded_receipt_and_validates_independently(
    ...
    assert (
        repository
        / "results/shape-placement-factorial-v32-smoke"
...
def test_campaign_requires_independently_passing_n31_coverage_smoke(
    ...
    assert not (
        repository / "results/shape-placement-factorial-v32"
    ).exists()
