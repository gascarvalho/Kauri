...
MANIFEST = REPOSITORY / "experiments/adaptive/profiles/shape-placement-factorial-v22.json"
V21_MANIFEST = (
...
def test_smoke_preflight_rejects_runtime_identity_drift(
    ...
    refusal = json.loads(capsys.readouterr().err)
    assert refusal == {
        "reason": "smoke runtime bytes differ from the exact frozen v22 identity",
        "status": "REJECT",
    }
...
def test_run_requires_explicit_authorization_before_any_launch(
    ...
    assert not (repository / "results/shape-placement-factorial-v22").exists()
...
def test_smoke_runtime_identity_drift_rejects_before_result_claim(
    ...
    assert not (
        repository / "results/shape-placement-factorial-v22-smoke"
    ).exists()
...
def test_coverage_smoke_runtime_identity_drift_rejects_before_result_claim(
    ...
    assert not (
        repository / "results/shape-placement-factorial-v22-coverage-smoke"
    ).exists()
...
def test_smoke_generates_exact_excluded_receipt_and_validates_independently(
    ...
    assert (
        repository
        / "results/shape-placement-factorial-v22-smoke"
...
def test_campaign_requires_independently_passing_n31_coverage_smoke(
    ...
    assert not (
        repository / "results/shape-placement-factorial-v22"
    ).exists()
