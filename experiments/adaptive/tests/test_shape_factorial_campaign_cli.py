"""Safe CLI-driver tests for the prospective one-shot SHAPE40 campaign."""
...
MANIFEST = REPOSITORY / "experiments/adaptive/profiles/shape-placement-factorial-v40.json"
V39_MANIFEST = (
...
def candidate_v40_identities():
    """Independently recompute prospective v40 identities without freezing them."""
    ...
    for module in (manifest_module, runtime_module, cli, validation_module):
        for name, value in identities.items():
            if hasattr(module, name):
                patcher.setattr(module, name, value)
...
def frozen_contract(candidate_v40_identities):
...
def test_prior_manifest_is_validation_only_before_any_result_claim(
    ...
    assert refusal == {
        "reason": (
            "shape-placement-factorial-v1 through v39 are validation-only; "
            "production commands require shape-placement-factorial-v40"
        ),
        "status": "REJECT",
    }
...
def test_smoke_preflight_rejects_runtime_identity_drift(
    ...
    refusal = json.loads(capsys.readouterr().err)
    assert refusal == {
        "reason": "smoke runtime bytes differ from the exact frozen v40 identity",
        "status": "REJECT",
    }
...
def test_coverage_smoke_preflight_validates_exact_n31_runtime(
    ...
    assert result["runtime_id"] == (
        "shape-placement-factorial-v40-excluded-n31-coverage-smoke-v1"
    )
...
def test_coverage_smoke_preflight_rejects_runtime_identity_drift(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    assert refusal == {
        "reason": (
            "coverage-smoke runtime bytes differ from the exact frozen v40 identity"
        ),
        "status": "REJECT",
    }
...
def test_run_requires_explicit_authorization_before_any_launch(
    ...
    assert not (repository / "results/shape-placement-factorial-v40").exists()
...
def test_smoke_runtime_identity_drift_rejects_before_result_claim(
    ...
    assert not (
        repository / "results/shape-placement-factorial-v40-smoke"
    ).exists()
...
def test_coverage_smoke_runtime_identity_drift_rejects_before_result_claim(
    ...
    assert not (
        repository / "results/shape-placement-factorial-v40-coverage-smoke"
    ).exists()
...
def test_smoke_generates_exact_excluded_receipt_and_validates_independently(
    ...
    assert (
        repository
        / "results/shape-placement-factorial-v40-smoke"
...
def test_coverage_smoke_uses_exact_first_slot_and_separate_authorization(
    ...
    expected_slots[1] = replace(
        expected_slots[1],
        byzantine=replace(expected_slots[1].byzantine, duration_s=360),
    )
...
def test_campaign_requires_independently_passing_n31_coverage_smoke(
    ...
    assert not (
        repository / "results/shape-placement-factorial-v40"
    ).exists()
...
def test_coverage_validation_uses_version_neutral_ordered_runtime_diagnostic(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    monkeypatch.setattr(
        cli,
        "_n31_coverage_smoke",
        lambda _plan: SimpleNamespace(runtime=object()),
    )
    ...
    assert cli.main(
        [
            ...
            str(MANIFEST),
            "--coverage-smoke-results-root",
            str(tmp_path / "coverage-smoke"),
        ]
    ) == 2
    refusal = json.loads(capsys.readouterr().err)

    assert refusal == {
        "reason": "ordered coverage validation lacks the ordered runtime",
        "status": "REJECT",
    }
