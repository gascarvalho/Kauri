...
def _inputs(
    ...
    *,
    ...
    source_profile = N7_PROFILE_V10 if mode == "smoke" else N31_PROFILE_V10
...
def test_default_cli_preflight_binds_checks_and_generated_issuer_into_auth_bytes(
    ...
    profile = runtime.load_focused_profile(N7_PROFILE_V10)
...
def test_cli_preflight_defaults_to_the_exact_v10_profile_and_proof(
    mode: str,
    pairs: int,
    profile_id: str,
    profile_sha256: str,
...
def test_cli_rejects_archived_v1_to_v9_profiles_for_new_execution(
    mode: str,
    profile_path: Path,
    pairs: int,
    tmp_path: Path,
    ...
    """Archived v1-v7 evidence remains readable but cannot authorize a new run."""
...
def test_cli_rejects_rebound_noncanonical_v8_profile_for_preflight_and_execution(
    tmp_path: Path,
) -> None:
    runner = _runner()
    profile = json.loads(N31_PROFILE_V8.read_text(encoding="utf-8"))
    proof_path = runtime_fixture._topology_proof_path(N31_PROFILE_V8, profile)
