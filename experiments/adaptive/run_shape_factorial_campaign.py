...
DEFAULT_MANIFEST = (
    Path(__file__).resolve().parent / "profiles/shape-placement-factorial-v20.json"
)
...
def _require_frozen_artifacts(
    ...
    """Fail before any result claim if producer bytes drift from v20."""

    if (
        manifest.manifest_id != FROZEN_MANIFEST_ID
        or manifest.manifest_sha256 != FROZEN_MANIFEST_SHA256
    ):
        raise FactorialExecutionError("campaign production requires exact frozen v20")
    if plan.plan_sha256 != FROZEN_PLAN_SHA256:
        raise FactorialExecutionError(
            "campaign plan bytes differ from the exact frozen v20 identity"
        )
    payload = canonical_runtime_bytes(runtime)
    if _sha256(payload) != FROZEN_RUNTIME_SHA256:
        raise FactorialExecutionError(
            "campaign runtime bytes differ from the exact frozen v20 identity"
        )
    return payload
...
def _run_smoke(
    ...
    *,
    ...
    if _sha256(smoke_runtime_payload) != FROZEN_SMOKE_RUNTIME_SHA256:
        raise FactorialExecutionError(
            "smoke runtime bytes differ from the exact frozen v20 identity"
        )
...
def _run_coverage_smoke(
    ...
    *,
    ...
    if _sha256(coverage_runtime_payload) != FROZEN_COVERAGE_SMOKE_RUNTIME_SHA256:
        raise FactorialExecutionError(
            "coverage-smoke runtime bytes differ from the exact frozen v20 identity"
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if manifest.manifest_id != FROZEN_MANIFEST_ID:
            raise FactorialExecutionError(
                "shape-placement-factorial-v1 through v19 are validation-only; "
                "production commands require shape-placement-factorial-v20"
            )
