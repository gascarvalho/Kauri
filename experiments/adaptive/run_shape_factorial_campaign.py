#!/usr/bin/env python3
"""Plan, execute once, or independently validate the frozen SHAPE29 campaign."""
...
DEFAULT_MANIFEST = (
    Path(__file__).resolve().parent / "profiles/shape-placement-factorial-v29.json"
)
...
def _n31_coverage_smoke(plan: FactorialPlan) -> N31CoverageSmokeSlot:
    ...
    repair = (
        ...
        if plan.manifest_id
        in {V25_MANIFEST_ID, V26_MANIFEST_ID, V27_MANIFEST_ID, V28_MANIFEST_ID, FROZEN_MANIFEST_ID}
        else None
    )
...
def _require_frozen_artifacts(
    ...
    """Fail before any result claim if producer bytes drift from v29."""

    if (
        manifest.manifest_id != FROZEN_MANIFEST_ID
        or manifest.manifest_sha256 != FROZEN_MANIFEST_SHA256
    ):
        raise FactorialExecutionError("campaign production requires exact frozen v29")
    if plan.plan_sha256 != FROZEN_PLAN_SHA256:
        raise FactorialExecutionError(
            "campaign plan bytes differ from the exact frozen v29 identity"
        )
    payload = canonical_runtime_bytes(runtime)
    if _sha256(payload) != FROZEN_RUNTIME_SHA256:
        raise FactorialExecutionError(
            "campaign runtime bytes differ from the exact frozen v29 identity"
        )
    return payload
    ...
    *,
    ...
    if len(slot_ids) == 2:
        if (
            ...
            raise FactorialExecutionError(
                "v29 coverage-smoke gate lacks its ordered runtime/static contract"
            )
...
def _run_smoke(
    ...
    *,
    ...
    if _sha256(smoke_runtime_payload) != FROZEN_SMOKE_RUNTIME_SHA256:
        raise FactorialExecutionError(
            "smoke runtime bytes differ from the exact frozen v29 identity"
        )
...
def _run_coverage_smoke(
    ...
    *,
    ...
    if _sha256(coverage_runtime_payload) != FROZEN_COVERAGE_SMOKE_RUNTIME_SHA256:
        raise FactorialExecutionError(
            "coverage-smoke runtime bytes differ from the exact frozen v29 identity"
        )
    ...
    if not isinstance(coverage.runtime, N31CoverageSmokeRuntime):
        raise FactorialExecutionError(
            "v29 coverage-smoke launch lacks the ordered runtime"
        )
...
def _run_campaign(
    ...
    *,
    ...
    if not isinstance(coverage.runtime, N31CoverageSmokeRuntime):
        raise FactorialExecutionError(
            "v29 campaign gate lacks the ordered coverage-smoke runtime"
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
            ...
            if arguments.command == "validate-coverage-smoke":
                ...
                if manifest.manifest_id in {
                    ...
                    if not isinstance(
                        ...
                        raise FactorialExecutionError(
                            "v29 coverage validation lacks the ordered runtime"
                        )
        ...
        if manifest.manifest_id != FROZEN_MANIFEST_ID:
            raise FactorialExecutionError(
                "shape-placement-factorial-v1 through v28 are validation-only; "
                "production commands require shape-placement-factorial-v29"
            )
        plan = build_factorial_plan(manifest)
