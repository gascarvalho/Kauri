#!/usr/bin/env python3
"""Plan, execute once, or independently validate the frozen SHAPE35 campaign."""
...
DEFAULT_MANIFEST = (
    Path(__file__).resolve().parent / "profiles/shape-placement-factorial-v35.json"
)
...
def _require_frozen_artifacts(
    ...
    """Fail before any result claim if producer bytes drift from v35."""

    if (
        manifest.manifest_id != FROZEN_MANIFEST_ID
        or manifest.manifest_sha256 != FROZEN_MANIFEST_SHA256
    ):
        raise FactorialExecutionError("campaign production requires exact frozen v35")
    if plan.plan_sha256 != FROZEN_PLAN_SHA256:
        raise FactorialExecutionError(
            "campaign plan bytes differ from the exact frozen v35 identity"
        )
    payload = canonical_runtime_bytes(runtime)
    if _sha256(payload) != FROZEN_RUNTIME_SHA256:
        raise FactorialExecutionError(
            "campaign runtime bytes differ from the exact frozen v35 identity"
        )
    return payload
    ...
    *,
    ...
    if len(slot_ids) == 2:
        if (
            ...
            raise FactorialExecutionError(
                "v35 coverage-smoke gate lacks its ordered runtime/static contract"
            )
...
def _run_smoke(
    ...
    *,
    ...
    if _sha256(smoke_runtime_payload) != FROZEN_SMOKE_RUNTIME_SHA256:
        raise FactorialExecutionError(
            "smoke runtime bytes differ from the exact frozen v35 identity"
        )
...
def _run_coverage_smoke(
    ...
    *,
    ...
    if not isinstance(coverage.runtime, N31CoverageSmokeRuntime):
        raise FactorialExecutionError(
            "v35 coverage-smoke launch lacks the ordered runtime"
        )
...
def _run_campaign(
    ...
    *,
    ...
    if not isinstance(coverage.runtime, N31CoverageSmokeRuntime):
        raise FactorialExecutionError(
            "v35 campaign gate lacks the ordered coverage-smoke runtime"
        )
            ...
            if arguments.command == "validate-coverage-smoke":
                ...
                if manifest.manifest_id in {
                    ...
                    if not isinstance(
                        ...
                        raise FactorialExecutionError(
                            "v35 coverage validation lacks the ordered runtime"
                        )
