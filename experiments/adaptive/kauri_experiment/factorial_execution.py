"""Fail-closed local execution for one SHAPE46 factorial slot.
...
def _bind_static_artifacts(
    ...
    *,
    ...
    try:
        derived_spec = build_slot_runtime(
            slot,
            excluded_repair_smoke_probe=spec.excluded_repair_smoke_probe,
            cycle1_responsive_cross_commit_retention=(
                spec.cycle1_responsive_cross_commit_retention
            ),
        )
...
def _read_commit_observer_event_stream(
    spec: SlotRuntimeSpec,
    slot_directory: Path,
    *,
    allow_partial: bool,
) -> tuple[_Event, ...]:
    """Read only the authoritative commit observer's structured stream."""
    ...
    if not source.startswith(prefix):
        raise FactorialExecutionError(
            "commit observer is not an exact replica source"
        )
    replica_text = source.removeprefix(prefix)
    try:
        replica_id = int(replica_text)
    except ValueError as error:
        raise FactorialExecutionError(
            "commit observer replica identity is malformed"
        ) from error
    if (
        source != f"replica-{replica_id}"
        or not 0 <= replica_id < spec.replica_count
    ):
        raise FactorialExecutionError(
            "commit observer replica identity is out of range"
        )
...
def observe_slot_phases(
    ...
    *,
    ...
    def drain_commit() -> _Event | None:
        candidates = [
            event
            for event in _read_commit_observer_event_stream(
                spec,
                slot_directory,
                allow_partial=True,
            )
...
def _uses_exact_excluded_repair_smoke_bound(
    ...
    expected_observation_contract = (
        EXCLUDED_REPAIR_SMOKE_OBSERVATION_CONTRACT_V6
        if manifest_version in {"v41", "v42", "v43", "v44", "v45", "v46"}
    ...
    expected_post_final_convergence_unmatched_commit_evidence_contract = (
        POST_FINAL_CONVERGENCE_UNMATCHED_COMMIT_EVIDENCE_CONTRACT_V2
        if manifest_version in {"v38", "v39", "v40", "v41", "v42", "v43", "v44", "v45", "v46"}
        else (
            POST_FINAL_CONVERGENCE_UNMATCHED_COMMIT_EVIDENCE_CONTRACT_V1
    ...
    expected_effective_fault_window_duration_s = (
        360
        if manifest_version in {"v40", "v41", "v42", "v43", "v44", "v45", "v46"}
        else 330
    ...
    exact_probe = (
        ...
        and probe.semantic_delta == expected_semantic_delta
        and probe.source_fault_window_duration_s == 450
        and probe.effective_fault_window_duration_s
        == expected_effective_fault_window_duration_s
        ...
        == (True if manifest_version in {"v40", "v41", "v42", "v43", "v44", "v45", "v46"} else None)
        ...
        == (2 if manifest_version in {"v40", "v41", "v42", "v43", "v44", "v45", "v46"} else None)
    ...
    exact_cycle1_gate_argv = (
        spec.manager_argv_template.argv.count(
            CYCLE1_SELECTION_NOT_BEFORE_OPTION
        )
        == (1 if manifest_version in {"v39", "v40", "v41", "v42", "v43", "v44", "v45", "v46"} else 0)
        and (
            manifest_version not in {"v39", "v40", "v41", "v42", "v43", "v44", "v45", "v46"}
    ...
    exact_readiness_argv_and_config = (
        spec.manager_argv_template.argv.count(
            CYCLE1_INHERITED_WAIT_EXEMPT_ELIGIBILITY_GATE_OPTION
        )
        == (1 if manifest_version in {"v40", "v41", "v42", "v43", "v44", "v45", "v46"} else 0)
        ...
        == (1 if manifest_version in {"v40", "v41", "v42", "v43", "v44", "v45", "v46"} else 0)
        and (
            manifest_version not in {"v40", "v41", "v42", "v43", "v44", "v45", "v46"}
            or spec.manager_argv_template.argv[
                spec.manager_argv_template.argv.index(
                    CYCLE1_RESPONSIVE_DEGRADED_ACTORS_OPTION
                )
        ...
        == (1 if manifest_version in {"v42", "v43", "v44", "v45", "v46"} else 0)
        and (
            manifest_version not in {"v42", "v43", "v44", "v45", "v46"}
            or spec.manager_argv_template.argv[
                spec.manager_argv_template.argv.index(
                    CYCLE1_RESPONSIVE_CROSS_COMMIT_RETENTION_ADMISSION_POLICY_OPTION
                )
        ...
        and (
            ...
            if manifest_version not in {"v42", "v43", "v44", "v45", "v46"}
    ...
    if not (
        ...
        and spec.fault_window.duration_s
        == expected_effective_fault_window_duration_s
        ...
        == EXCLUDED_REPAIR_SMOKE_VERIFIED_RESPONSE_DUPLICATE_PROBE_CONTRACT_V1
        and spec.causal_acceptance.post_final_convergence_unmatched_commit_evidence_contract
        == expected_post_final_convergence_unmatched_commit_evidence_contract
        and exact_probe
        and exact_replica_argv
        and exact_cycle1_gate_argv
        and exact_readiness_argv_and_config
    ):
        raise FactorialExecutionError(
            "v28/v29/v30/v31/v32/v33/v34/v35/v36/v37/v38/v39/v40/v41/v42/v43/v44/v45/v46 excluded repair smoke runtime binding drifted"
        )
    return True
...
def _assert_excluded_repair_smoke_completion(
    ...
    *,
    ...
    expected_observation_contract = (
        spec.causal_acceptance.excluded_repair_smoke_observation_contract
    )
    if expected_observation_contract not in {
        EXCLUDED_REPAIR_SMOKE_OBSERVATION_CONTRACT_V1,
        EXCLUDED_REPAIR_SMOKE_OBSERVATION_CONTRACT_V2,
        EXCLUDED_REPAIR_SMOKE_OBSERVATION_CONTRACT_V3,
        EXCLUDED_REPAIR_SMOKE_OBSERVATION_CONTRACT_V4,
        ...
        raise FactorialExecutionError(
            "excluded repair smoke observation contract is unknown"
        )
    if (
    ...
    cycle1_selection_lower_bound_ns = expected_fault_end_ns
    if (
        expected_observation_contract
        in {
            EXCLUDED_REPAIR_SMOKE_OBSERVATION_CONTRACT_V4,
        ...
        cycle1_selection_lower_bound_ns += (
            EXCLUDED_REPAIR_SMOKE_POST_FAULT_OBSERVATION_GRACE_S
            * NANOSECONDS_PER_SECOND
        )
    if not (
        cycle1_selection_lower_bound_ns < timestamps[6]
        and cycle1_selection_lower_bound_ns < timestamps[7]
    ):
        raise IncompleteFactorialSlot(
            (
                "excluded repair cycle-1 selection lower bound must be "
                "strictly before selection and command"
            )
            if expected_observation_contract
            in {
                EXCLUDED_REPAIR_SMOKE_OBSERVATION_CONTRACT_V4,
                EXCLUDED_REPAIR_SMOKE_OBSERVATION_CONTRACT_V5,
                EXCLUDED_REPAIR_SMOKE_OBSERVATION_CONTRACT_V6,
                ...
                "excluded repair fault end must be strictly before cycle-1 "
...
def _assert_v40_repair_runner_terminal_before_hard_deadline(
    spec: SlotRuntimeSpec,
    *,
    shared_raw_clock_anchor_ns: int,
    runner_terminal_recorded_monotonic_ns: int,
) -> None:
    """Bind an exact v40+ repair terminal row to the shared hard deadline."""

    probe = spec.excluded_repair_smoke_probe
    retention = spec.cycle1_responsive_cross_commit_retention
    probe_contract = (
        (probe.observation_contract, probe.semantic_delta)
        if probe is not None
        else None
    )
    expected_result_paths: tuple[str, ...] = ()
    if probe_contract == (
        EXCLUDED_REPAIR_SMOKE_OBSERVATION_CONTRACT_V5,
        EXCLUDED_REPAIR_SMOKE_SEMANTIC_DELTA_V4,
        ...
        if retention is None:
        ...
        elif retention == Cycle1ResponsiveCrossCommitRetentionContract(
            scope=EXCLUDED_REPAIR_S037_RETENTION_SCOPE_V1,
            observation_schema_version=2,
            ...
            expected_result_paths = {
                ...
                (
                    "results/shape-placement-factorial-v45/"
                    "slot-037-n31-f2-b04-00"
                ): (
                ...
                (
                    "results/shape-placement-factorial-v46/"
                    "slot-037-n31-f2-b04-00"
                ): (
    ...
    if (
        probe is None
        or not expected_result_paths
        or probe.runner_terminal_strictly_before_hard_deadline is not True
        or spec.slot_id != "slot-037-n31-f2-b04-00"
        or spec.result_path not in expected_result_paths
        or spec.fault_window.duration_s != 360
        or spec.fault_window.hard_timeout_s != 650
    ):
        raise FactorialExecutionError(
            "runner terminal hard bound requires the exact v40/v41/v42/v43/v44/v45/v46 repair runtime"
        )
    if (
        type(shared_raw_clock_anchor_ns) is not int
        or shared_raw_clock_anchor_ns <= 0
        or type(runner_terminal_recorded_monotonic_ns) is not int
        or runner_terminal_recorded_monotonic_ns <= 0
    ):
        raise FactorialExecutionError(
            "runner terminal hard bound requires positive monotonic timestamps"
        )
    hard_deadline_ns = shared_raw_clock_anchor_ns + (
        spec.fault_window.hard_timeout_s * NANOSECONDS_PER_SECOND
    )
    if hard_deadline_ns > MAX_MONOTONIC_NS:
        raise FactorialExecutionError(
            "runner terminal hard deadline exceeds uint64"
        )
    if runner_terminal_recorded_monotonic_ns >= hard_deadline_ns:
        raise IncompleteFactorialSlot(
            "runner terminal must be strictly before the shared hard deadline"
        )


def _finalize_runner_terminal(
    spec: SlotRuntimeSpec,
    *,
    coverage_binding: CoverageSmokeLaunchBinding | None,
    shared_raw_clock_anchor_ns: int | None,
    ...
    """Sample, classify, and persist one exact terminal timestamp."""

    recorded_monotonic_ns = raw_now_ns()
    if type(recorded_monotonic_ns) is not int or recorded_monotonic_ns <= 0:
        runtime_error = runtime_error or (
            "runner terminal requires a positive monotonic timestamp"
        )
    if (
        runtime_error is None
        and cleanup_error is None
        and spec.excluded_repair_smoke_probe is not None
        and spec.excluded_repair_smoke_probe.runner_terminal_strictly_before_hard_deadline
        is True
    ):
        try:
            if shared_raw_clock_anchor_ns is None:
                raise FactorialExecutionError(
                    "v40/v41/v42/v43/v44/v45/v46 runner terminal hard bound lacks shared anchor"
                )
            if not _uses_exact_excluded_repair_smoke_bound(spec, coverage_binding):
                raise FactorialExecutionError(
                    "v40/v41/v42/v43/v44/v45/v46 runner terminal hard bound lacks exact coverage binding"
                )
            _assert_v40_repair_runner_terminal_before_hard_deadline(
                spec,
                shared_raw_clock_anchor_ns=shared_raw_clock_anchor_ns,
                runner_terminal_recorded_monotonic_ns=recorded_monotonic_ns,
            )
        except FactorialExecutionError as error:
            runtime_error = str(error)
    terminal = (
        "PASS" if runtime_error is None and cleanup_error is None else "INCOMPLETE"
    )
    ...
    return terminal, reason, recorded_monotonic_ns
    ...
    *,
    ...
    cycle1_gate_enabled = (
        ...
        and (
            ...
            or (
                probe.semantic_delta == EXCLUDED_REPAIR_SMOKE_SEMANTIC_DELTA_V4
                and probe.observation_contract
                == EXCLUDED_REPAIR_SMOKE_OBSERVATION_CONTRACT_V5
                and spec.result_path
                == (
            ...
            or (
                probe.semantic_delta == EXCLUDED_REPAIR_SMOKE_SEMANTIC_DELTA_V5
                and probe.observation_contract
                == EXCLUDED_REPAIR_SMOKE_OBSERVATION_CONTRACT_V6
                and spec.result_path
                in {
    ...
    retention = spec.cycle1_responsive_cross_commit_retention
    historical_repair_readiness_enabled = (
        retention is None
        and cycle1_gate_enabled
        and probe is not None
        and (probe.observation_contract, probe.semantic_delta) in {
            (
                EXCLUDED_REPAIR_SMOKE_OBSERVATION_CONTRACT_V5,
                EXCLUDED_REPAIR_SMOKE_SEMANTIC_DELTA_V4,
            ),
    ...
    if cycle1_gate_enabled:
        cycle1_gate_ns = end_ns + (
            EXCLUDED_REPAIR_SMOKE_POST_FAULT_OBSERVATION_GRACE_S
            * NANOSECONDS_PER_SECOND
        )
        if (
            manager_argv.count(CYCLE1_SELECTION_NOT_BEFORE_OPTION) != 1
            or manager_argv[
                manager_argv.index(CYCLE1_SELECTION_NOT_BEFORE_OPTION) + 1
            ]
            != str(cycle1_gate_ns)
        ):
            raise FactorialExecutionError(
                "launch receipt cannot seal drifted repair cycle-1 selection gate"
            )
    elif CYCLE1_SELECTION_NOT_BEFORE_OPTION in manager_argv:
        raise FactorialExecutionError(
            "non-repair launch receipt must not seal a cycle-1 selection gate"
        )
    readiness_options = (
        CYCLE1_INHERITED_WAIT_EXEMPT_ELIGIBILITY_GATE_OPTION,
        CYCLE1_RESPONSIVE_CROSS_COMMIT_RETENTION_READINESS_GATE_OPTION,
    ...
    main_config_rows = tuple(
        row
        for row in materialized.input_artifacts
        if row.get("kind") == "main_config"
        or row.get("relative_path") == spec.main_config.path
    )
    main_config_sha256: str | None = None
    readiness_enabled = historical_repair_readiness_enabled or retention is not None
    if readiness_enabled:
        ...
        inherited_required = historical_repair_readiness_enabled or (
            retention is not None
            and retention.scope == EXCLUDED_REPAIR_S037_RETENTION_SCOPE_V1
        )
        replica_config_values = tuple(
            tuple(
        ...
        replica_config_paths = {
            values[0] for values in replica_config_values if len(values) == 2
        }
        materialized_main_config = (
            Path(next(iter(replica_config_paths)))
            if len(replica_config_paths) == 1
            else None
        )
        materialized_main_payload: bytes | None = None
        materialized_main_lines: tuple[str, ...] = ()
        if (
            materialized_main_config is not None
            and materialized_main_config.is_file()
            and not materialized_main_config.is_symlink()
        ):
            try:
                materialized_main_payload = materialized_main_config.read_bytes()
                materialized_main_lines = tuple(
                    materialized_main_payload.decode("utf-8").splitlines()
                )
            except (OSError, UnicodeDecodeError):
                materialized_main_payload = None
                materialized_main_lines = ()
        materialized_v2_key_lines = tuple(
            line
            for line in materialized_main_lines
            if line.split("=", 1)[0].strip()
            == RESPONSIVE_CROSS_COMMIT_RETENTION_V2_CONFIG_KEY
        )
        if (
            (manager_argv.count(readiness_options[0]) == 1)
            is not inherited_required
            or manager_argv.count(readiness_options[1]) != 1
            or manager_argv.count(readiness_options[2]) != 1
            or manager_argv[manager_argv.index(readiness_options[2]) + 1]
            != ",".join(map(str, expected_actors))
            or (manager_argv.count(readiness_options[3]) == 1)
            is not (expected_policy is not None)
            or (
                expected_policy is not None
                and manager_argv[manager_argv.index(readiness_options[3]) + 1]
                != expected_policy
            )
            ...
            != 1
            or len(main_config_rows) != 1
            or main_config_rows[0].get("kind") != "main_config"
            or main_config_rows[0].get("relative_path") != spec.main_config.path
            or set(main_config_rows[0])
            != {"kind", "replica_id", "relative_path", "sha256", "size_bytes"}
            or main_config_rows[0].get("replica_id") is not None
            or not isinstance(main_config_rows[0].get("sha256"), str)
            or len(str(main_config_rows[0].get("sha256"))) != 64
            or any(
                character not in _HEX_DIGITS
                for character in str(main_config_rows[0].get("sha256"))
            )
            or len(materialized.redacted_replica_argv) != spec.replica_count
            or any(
                len(values) != 2
                or Path(values[1])
                != Path(values[0]).parent / f"replica-{process.replica_id}.conf"
                for process, values in zip(
                    materialized.redacted_replica_argv,
                    replica_config_values,
                    strict=True,
                )
            )
            or tuple(
            ...
            != tuple(range(spec.replica_count))
            or materialized_main_config is None
            or materialized_main_config.name != "main.conf"
            or materialized_main_config.parent.name != "runtime"
            or materialized_main_payload is None
            or not materialized_main_payload.endswith(b"\n")
            or b"\r" in materialized_main_payload
            or materialized_main_lines.count(
                RESPONSIVE_CROSS_COMMIT_RETENTION_V2_CONFIG_LINE
            )
            != 1
            or materialized_v2_key_lines
            != (RESPONSIVE_CROSS_COMMIT_RETENTION_V2_CONFIG_LINE,)
            or main_config_rows[0].get("size_bytes")
            != len(materialized_main_payload)
            or hashlib.sha256(materialized_main_payload).hexdigest()
            != main_config_rows[0].get("sha256")
        ):
            raise FactorialExecutionError(
                "launch receipt cannot seal drifted reporter readiness or main config"
            )
        main_config_sha256 = str(main_config_rows[0]["sha256"])
    elif (
        any(option in manager_argv for option in readiness_options)
        or RESPONSIVE_CROSS_COMMIT_RETENTION_V2_CONFIG_LINE
        in spec.main_config.lines
    ):
        raise FactorialExecutionError(
            "ungated launch receipt must not seal readiness options"
        )
    receipt = {
        "schema_version": 1,
        "slot_id": spec.slot_id,
    ...
    if main_config_sha256 is not None:
        receipt["main_config_sha256"] = main_config_sha256
    if retention is not None:
        receipt["cycle1_responsive_cross_commit_retention"] = (
            retention.as_document()
        )
    return receipt
    ...
    *,
    ...
    def state(
        ...
        **extra: object,
        ...
        if type(recorded_monotonic_ns) is not int or recorded_monotonic_ns < 0:
            raise FactorialExecutionError(
                "runner state requires a nonnegative monotonic timestamp"
            )
        _append_jsonl(
            state_path,
            {
    ...
    terminal, reason, _ = _finalize_runner_terminal(
        spec,
...
def build_n31_coverage_smoke_slot(
    ...
    *,
    ...
    frozen_campaign_paths = {
        ...
        "results/shape-placement-factorial-v29/slot-066-n31-f5-b05-P": "v29",
        "results/shape-placement-factorial-v30/slot-066-n31-f5-b05-P": "v30",
        "results/shape-placement-factorial-v31/slot-066-n31-f5-b05-P": "v31",
        "results/shape-placement-factorial-v32/slot-066-n31-f5-b05-P": "v32",
        "results/shape-placement-factorial-v33/slot-066-n31-f5-b05-P": "v33",
        "results/shape-placement-factorial-v34/slot-066-n31-f5-b05-P": "v34",
        ...
        "results/shape-placement-factorial-v39/slot-066-n31-f5-b05-P": "v39",
        "results/shape-placement-factorial-v40/slot-066-n31-f5-b05-P": "v40",
        ...
        "results/shape-placement-factorial-v45/slot-066-n31-f5-b05-P": "v45",
        "results/shape-placement-factorial-v46/slot-066-n31-f5-b05-P": "v46",
    }
    manifest_version = frozen_campaign_paths.get(template.result_path)
    expected_fault_duration_s = (
        450
        if manifest_version
        in {"v27", "v28", "v29", "v30", "v31", "v32", "v33", "v34", "v35", "v36", "v37", "v38", "v39", "v40", "v41", "v42", "v43", "v44", "v45", "v46"}
        else 300
    )
    ...
    expected_timeout_eligibility = {
        ...
        "v29": RESPONSIVE_CAUSAL_TIMEOUT_ELIGIBILITY_V3,
        "v30": RESPONSIVE_CAUSAL_TIMEOUT_ELIGIBILITY_V3,
        "v31": RESPONSIVE_CAUSAL_TIMEOUT_ELIGIBILITY_V3,
        "v32": RESPONSIVE_CAUSAL_TIMEOUT_ELIGIBILITY_V3,
        "v33": RESPONSIVE_CAUSAL_TIMEOUT_ELIGIBILITY_V3,
        "v34": RESPONSIVE_CAUSAL_TIMEOUT_ELIGIBILITY_V3,
        ...
        "v39": RESPONSIVE_CAUSAL_TIMEOUT_ELIGIBILITY_V3,
        "v40": RESPONSIVE_CAUSAL_TIMEOUT_ELIGIBILITY_V3,
        ...
        "v45": RESPONSIVE_CAUSAL_TIMEOUT_ELIGIBILITY_V3,
        "v46": RESPONSIVE_CAUSAL_TIMEOUT_ELIGIBILITY_V3,
    ...
    expected_future_tree_proposal_delivery_contract = (
        FUTURE_TREE_PROPOSAL_DELIVERY_CONTRACT_V3
        if manifest_version == "v46"
        else FUTURE_TREE_PROPOSAL_DELIVERY_CONTRACT_V2
        if manifest_version
        in {"v23", "v24", "v25", "v26", "v27", "v28", "v29", "v30", "v31", "v32", "v33", "v34", "v35", "v36", "v37", "v38", "v39", "v40", "v41", "v42", "v43", "v44", "v45"}
        else (
            FUTURE_TREE_PROPOSAL_DELIVERY_CONTRACT_V1
    ...
    expected_evidence_snapshot_selection_contract = (
        EVIDENCE_SNAPSHOT_SELECTION_CONTRACT_V1
        if manifest_version
        in {"v22", "v23", "v24", "v25", "v26", "v27", "v28", "v29", "v30", "v31", "v32", "v33", "v34", "v35", "v36", "v37", "v38", "v39", "v40", "v41", "v42", "v43", "v44", "v45", "v46"}
        else None
    )
    expected_inherited_wait_exempt_placement_contract = (
        INHERITED_CONSENSUS_WAIT_EXEMPT_PLACEMENT_CONTRACT_V1
        if manifest_version in {"v25", "v26", "v27", "v28", "v29", "v30", "v31", "v32", "v33", "v34", "v35", "v36", "v37", "v38", "v39", "v40", "v41", "v42", "v43", "v44", "v45", "v46"}
        else None
    )
    expected_verified_response_duplicate_delivery_contract = (
        VERIFIED_RESPONSE_DUPLICATE_DELIVERY_CONTRACT_V2
        if manifest_version in {"v27", "v28", "v29", "v30", "v31", "v32", "v33", "v34", "v35", "v36", "v37", "v38", "v39", "v40", "v41", "v42", "v43", "v44", "v45", "v46"}
        else (
            VERIFIED_RESPONSE_DUPLICATE_DELIVERY_CONTRACT_V1
    ...
    expected_excluded_repair_observation_contract = (
        ...
        if manifest_version in {"v34", "v35", "v36"}
            ...
            EXCLUDED_REPAIR_SMOKE_OBSERVATION_CONTRACT_V1
            if manifest_version in {"v28", "v29", "v30", "v31", "v32", "v33"}
            else None
        )
    )
    expected_excluded_repair_duplicate_probe_contract = (
        EXCLUDED_REPAIR_SMOKE_VERIFIED_RESPONSE_DUPLICATE_PROBE_CONTRACT_V1
        if manifest_version in {"v28", "v29", "v30", "v31", "v32", "v33", "v34", "v35", "v36", "v37", "v38", "v39", "v40", "v41", "v42", "v43", "v44", "v45", "v46"}
        else None
    )
    expected_post_final_convergence_unmatched_commit_evidence_contract = (
        POST_FINAL_CONVERGENCE_UNMATCHED_COMMIT_EVIDENCE_CONTRACT_V2
    ...
    if (
        ...
        != (
            RESPONSIVE_CAUSAL_SELECTION_REPORTER_RETENTION_READINESS_GATE_V2
            if manifest_version in {"v44", "v45", "v46"}
        ...
        != expected_excluded_repair_duplicate_probe_contract
        or responsive.post_final_convergence_unmatched_commit_evidence_contract
        != expected_post_final_convergence_unmatched_commit_evidence_contract
        or template.workload.epoch1_preselection_residency_ms
        != (
            60_000
            if manifest_version
            in {"v24", "v25", "v26", "v27", "v28", "v29", "v30", "v31", "v32", "v33", "v34", "v35", "v36", "v37", "v38", "v39", "v40", "v41", "v42", "v43", "v44", "v45", "v46"}
            else None
        )
    ...
    primary_runtime = build_slot_runtime(
        smoke,
        cycle1_responsive_cross_commit_retention=(
            Cycle1ResponsiveCrossCommitRetentionContract(
                scope=ORDERED_S066_COVERAGE_RETENTION_SCOPE_V1,
                observation_schema_version=2,
                responsive_degraded_actor_ids=(
                    smoke.responsive_degraded_actor_ids
                ),
                admission_policy=(
                    AGGREGATE_RELAY_PER_ACTOR_ADMISSION_POLICY_V1
                ),
            ...
            if manifest_version in {"v42", "v43", "v44", "v45", "v46"}
            else None
        ),
    )
    if manifest_version not in {"v25", "v26", "v27", "v28", "v29", "v30", "v31", "v32", "v33", "v34", "v35", "v36", "v37", "v38", "v39", "v40", "v41", "v42", "v43", "v44", "v45", "v46"}:
    ...
    if (
        ...
        != expected_excluded_repair_duplicate_probe_contract
        or repair_responsive.post_final_convergence_unmatched_commit_evidence_contract
        != expected_post_final_convergence_unmatched_commit_evidence_contract
    ...
    if manifest_version in {"v28", "v29", "v30", "v31", "v32", "v33", "v34", "v35", "v36", "v37", "v38", "v39", "v40", "v41", "v42", "v43", "v44", "v45", "v46"}:
        source_repair_runtime = build_slot_runtime(
            ...
            cycle1_responsive_cross_commit_retention=(
                Cycle1ResponsiveCrossCommitRetentionContract(
                    ...
                    observation_schema_version=2,
                    responsive_degraded_actor_ids=(
                        repair_template.responsive_degraded_actor_ids
                    ),
                    admission_policy=(
                        AGGREGATE_RELAY_PER_ACTOR_ADMISSION_POLICY_V1
                    ),
        ...
        excluded_repair_probe = ExcludedRepairSmokeProbeContract(
            ...
            effective_fault_window_duration_s=effective_fault_window_duration_s,
            hard_timeout_s=650,
            observation_contract=expected_excluded_repair_observation_contract,
        ...
        repair_runtime = build_slot_runtime(
            ...
            cycle1_responsive_cross_commit_retention=(
                Cycle1ResponsiveCrossCommitRetentionContract(
                    ...
                    observation_schema_version=2,
                    responsive_degraded_actor_ids=(
                        repair_smoke.responsive_degraded_actor_ids
                    ),
                    admission_policy=(
                        ONE_PER_ACTOR_WITH_GLOBAL_AGGREGATE_ADMISSION_POLICY_V1
                    ),
